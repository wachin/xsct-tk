...
## 🌙  xsct_tk – Easy Screen Light Control, for X11 Window Manager
...
`xsct_tk` is a **graphical interface** for [xsct](https://github.com/faf0/sct), a small command-line tool that changes your screen’s color temperature using `xrandr`.
...
Alternatively run with launcher, make sure the `Launcher.sh` script is executable. In the file manager, right-click on it and in the "**Permissions**" tab make sure it "**is executable**"
...
On some Linux like MX Linux you can right click on the `xsct_tk.py` file and open it with python launcher.
...
Absolutely! The program is written in **Python**, so you can open `xsct_tk.py` in any text editor (like **Geany**, **Thonny**, or **Mousepad**) to:
- Change colors
- Edit labels
- Add new features
...
- 🐍 **GUI Source Code**: [https://github.com/wachin/xsct_tk](https://github.com/wachin/xsct_tk)
